...
class Ui_Index(object):
    def download(self):
        ...
        if ind == "":
            ...
            retval = msg.exec_()
            return
        in1 = nse.get_index_quote(ind)
        in_dic = {'Name': in1['name'], "Last Price": in1['lastPrice'], "Change": in1['change'],
                  "Percentage Change": in1['pChange']}
        in_pd = pd.DataFrame(pd.Series(in_dic))
        # print(in_pd)

        file_name = 'index.xlsx'
        z = pd.ExcelWriter(file_name)
        in_pd.to_excel(z)
        z.save()

        df = pd.read_excel(file_name)
        writer = pd.ExcelWriter('enhanced_index.xlsx', engine='xlsxwriter')
